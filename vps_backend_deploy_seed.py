...
commands = [
    ...
    "cd /root/realtorone/backend && if [ -d storage/app/public/course-assets ]; then mv storage/app/public/course-assets /tmp/; fi",
    "cd /root/realtorone/backend && docker compose up -d --build --remove-orphans",
    "cd /root/realtorone/backend && if [ -d /tmp/course-assets ]; then mv /tmp/course-assets storage/app/public/; fi",
